...
import { defaultCheckBoxSubTitle, defaultCheckboxData } from '../../../../../Data/Forms/FormControlsData'
...
export default function DefaultCheckbox() {
    ...
    return (
        <Col xs={12}>
            <Card>
                <CommonCardHeader headClass='pb-0' title={DefaultCheckboxTitle} subTitle={defaultCheckBoxSubTitle} />
                <CardBody>
                    <Row className="g-3">
                                ...
                                <div className="card-wrapper border rounded-3 checkbox-checked">
                                    ...
                                    {data.child.map((item) => (
                                        <FormGroup key={item.id} className={item.groupClass ? item.groupClass : ''} check>
                                            <Input id={item.id} type="checkbox" value='' defaultChecked={item.check === 'checked'} disabled={item.disabled === 'disabled'} />
