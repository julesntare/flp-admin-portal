...
import { popularBanksList } from '../../../../../Data/Forms/FormLayoutData';
        ...
        <Form onSubmit={(event) => event.preventDefault()} className="needs-validation" noValidate>
            <Row className='g-3'>
                <Col sm={6} className="bank-search">
                    ...
                    <Input value={aadhaarNumber} name="aadhaarNumber" onChange={updateFormData} type="search" placeholder="xxxx xxxx xxxx xxxx" />
                </Col>
                <Col sm={6} className="bank-search">
                    <Label>{PAN}<span className="txt-danger">*</span></Label>
                    <Input type="search" placeholder="xxxxxxxxxx" value={panNumber} name="panNumber" onChange={updateFormData} />
                </Col>
                <Col xs={12}>
                    <H6>{ChooseBanks}</H6>
                                ...
                                {popularBanksList.map((data, index) => (
                                    <LI key={index}>
                                        ...
                                        <Label htmlFor={data.bankName} check>
                                            <Image src={dynamicImage(`forms/${data.image}`)} alt="bank" body={true} />
                ...
                <Col xs={12} className="text-end">
                    <Btn color="primary me-2" onClick={() => activeCallBack(1)}>{Previous}</Btn>
